...
# define data column header strings
CAUSE_DEATH_STR = 'SEER cause-specific death classification'
SURVIVAL_MONTHS_STR = 'Survival months'
PATIENT_ID_STR = 'Patient ID'    
REC_NUM_STR = 'Record number recode'
YR_DX_STR = 'Year of diagnosis'
...
class Data:
    ...
    def filter_cases(self, cases_file):
        ...
        for idx, case in cases.iterrows():
            entry = (case[PATIENT_ID_STR], case[REC_NUM_STR])
            cases_set.add(entry)

        for idx, row in self.frame.iterrows():
            entry = (row[PATIENT_ID_STR], row[REC_NUM_STR])
            if entry in cases_set:
                cases_mask.append(False)
            else:
                cases_mask.append(True)

        cases_columns = np.arange(self.frame[SURVIVAL_MONTHS_STR].size)[cases_mask]
        self.frame = self.frame.drop(cases_columns)
        # Refresh indices starting from zero
        self.frame = self.frame.set_index(np.arange(self.frame[SURVIVAL_MONTHS_STR].size))
    ...
    def filter_start_date(self, date):
        ...
        self.frame = fc.constraint(self.frame, YR_DX_STR, operator.ge, date)
    ...
    def create_target(self, task):
        ...
        if task in ['mort12', 'mort60']:
            ...
            # you should use the SEER cause specific death classification and specify any of the cancer causes of death.
            self.frame = fc.constraint(self.frame, CAUSE_DEATH_STR, operator.eq, 1)
            if task == 'mort12':
                self.frame = fc.constraint(self.frame, SURVIVAL_MONTHS_STR, operator.le, 12)
            elif task == 'mort60':
                self.frame = fc.constraint(self.frame, SURVIVAL_MONTHS_STR, operator.le, 60)

            # Remove target - from now on encodings only for inputs.
            self.encodings.pop(SURVIVAL_MONTHS_STR)
            ...
            del self.frame[CAUSE_DEATH_STR]
            del self.frame[SURVIVAL_MONTHS_STR]
            self.encodings.pop(CAUSE_DEATH_STR)
            self.encodings.pop(SURVIVAL_MONTHS_STR)
    ...
    def create_survived_cancer_for_n_months(self, n):
        ...
        # Remove unknown survival
        self.frame = fc.constraint(self.frame, SURVIVAL_MONTHS_STR, operator.lt, 999)

        # Remove cases that died within n month of other cause
        early_other_death_mask =\
            (self.frame[SURVIVAL_MONTHS_STR] < n) & (self.frame[CAUSE_DEATH_STR] != 1)
        early_other_death_columns = np.arange(self.frame[SURVIVAL_MONTHS_STR].size)[early_other_death_mask]
        ...
        def survived_cancer_for_n_months(row):
            ...
            if row[SURVIVAL_MONTHS_STR] < n and row[CAUSE_DEATH_STR] == 1:
                return 0
